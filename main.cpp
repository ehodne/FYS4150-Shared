...
double potential_interact(double x, double w_r);
...
void initialize_interacting_case(mat& A, double rho_0, double rho_n, int n, double w_r); // Initialize the matrix for interaction case
...
int main(int argc, char* argv[])
{
    ...
    double rho_0 = 0;
    double rho_n = 10;
    //initialize_non_interacting_case(A, rho_0, rho_n, n);
    initialize_interacting_case(A,rho_0,rho_n,n,w_r);
...
//    }
    ...
    vec eigval = A.diag();
    eigval = sort(eigval);
    cout << "Minimal eigenvalue " << eigval(0) << endl;
    //cout << "Second eigenvalue " << eigval(1) << endl;
    //cout << "Third eigenvalue " << eigval(2) << endl;
...
void initialize_interacting_case(mat& A, double rho_0, double rho_n, int n, double w_r)
{
    // compute the step size
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < n; j++)
        {
            if (i==j)
            {
                ...
                A(i,j)  = 2/(h*h) + potential_interact(x, w_r);
            }
            else if (j==i+1)
            {
                // Upper diagonal elements
